...
<section class="my-12">
    <div class="container mx-auto max-w-md">
        <h1 class="text-3xl font-title text-heading text-center mb-6">{{ __('public/auth.login') }}</h1>

        <form action="{{ route('login') }}" method="POST" class="bg-white p-6 rounded-lg shadow-lg">
            @csrf
            <div class="mb-4">
                <label for="email" class="block text-sm font-medium text-gray-700">{{ __('public/auth.email') }}</label>
            ...
            <div class="mb-4">
                <label for="password" class="block text-sm font-medium text-gray-700">{{ __('public/auth.password') }}</label>
            ...
            <div class="flex items-center justify-between mb-4">
                <label class="flex items-center">
                    ...
                    <span class="ml-2 text-sm text-gray-600">{{ __('public/auth.remember_me') }}</span>
                </label>
                <a href="#" class="text-sm text-accent hover:underline">{{ __('public/auth.forgot_password') }}</a>
            </div>

            <div>
                <button type="submit" class="w-full px-4 py-2 bg-accent text-white font-bold rounded-lg hover:bg-cta transition">
                    {{ __('public/auth.login_button') }}
                </button>
            </div>
        </form>
    </div>
