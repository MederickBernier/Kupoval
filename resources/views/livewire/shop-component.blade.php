...
<div class="p-3 sm:p-6 bg-soft-aqua text-charcoal-gray min-h-screen">
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 sm:mb-6 gap-3">
        <h2 class="text-2xl sm:text-3xl font-bold text-navy-blue tracking-wide font-serif">
            {{ __('public/shop.browse_shop') }}</h2>
    ...
    @if ($filtersVisible)
        <div
            class="mb-4 sm:mb-6 p-4 sm:p-5 bg-gray-50 shadow-lg rounded-xl border border-deep-emerald transition-all duration-300">
            <!-- 🔹 Filters Header -->
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4 sm:mb-6">
                ...
                <div class="flex flex-col">
                    ...
                    <input type="number" wire:model.live="priceMin" min="0" step="10"
                ...
                <div class="flex flex-col">
                    ...
                    <input type="number" wire:model.live="priceMax" min="0" step="10"
                        class="border border-deep-emerald rounded-md px-4 py-2 w-full bg-white text-gray-800 shadow-sm"
                ...
                <div class="border border-deep-emerald rounded-xl p-4 shadow-sm bg-gray-100">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-32 overflow-y-auto p-2 bg-white rounded-md">
                        @foreach ($categories as $category)
                            <label
                                class="flex items-center space-x-2 px-2 py-1 cursor-pointer hover:bg-gray-200 transition">
                                <input type="checkbox" wire:model.live="selectedCategories"
                                    value="{{ (int) $category->id }}" class="w-5 h-5 accent-emerald-green">
                ...
                <div class="border border-deep-emerald rounded-xl p-4 shadow-sm bg-gray-100">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-32 overflow-y-auto p-2 bg-white rounded-md">
                        @foreach ($artists as $artist)
                            <label
                                class="flex items-center space-x-2 px-2 py-1 cursor-pointer hover:bg-gray-200 transition">
                                <input type="checkbox" wire:model.live="selectedArtist" value="{{ (int) $artist->id }}"
            ...
            <div
                ...
                <div class="relative">
                    <img src="{{ Storage::url($artwork->image) }}" alt="{{ $artwork->name }}"
                        class="w-full h-48 sm:h-64 object-cover transition-transform duration-300 group-hover:scale-105">

                    <!-- Wishlist Button -->
                    <button wire:click="toggleWishlist({{ $artwork->id }})"
                        class="absolute top-3 right-3 bg-black/60 backdrop-blur-sm text-white p-2 rounded-full hover:bg-teal-600 transition-colors duration-200">
                        @if (in_array($artwork->id, $wishlist))
                            <i class="bi bi-heart-fill text-red-400 text-lg"></i>
                        @else
                    ...
                    <!-- Price Badge -->
                    <div
                        class="absolute bottom-3 left-3 bg-gradient-to-r from-teal-600 to-emerald-500 text-white px-3 py-1.5 rounded-full text-sm font-semibold shadow-lg">
                ...
                <div class="p-4 sm:p-5">
                    <h3 class="font-title text-lg font-semibold text-heading mb-1 line-clamp-1">{{ $artwork->name }}
                    </h3>
                    ...
                    <div class="flex gap-2">
                        <button wire:click="addToCart({{ $artwork->id }})"
                            class="flex-1 bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white px-4 py-2.5 rounded-full text-sm font-medium transition-all duration-200 flex items-center justify-center gap-2 shadow-md hover:shadow-lg transform hover:-translate-y-0.5">
                            <i class="bi bi-cart-plus text-sm"></i>
                            <span>{{ __('public/shop.add_to_cart') }}</span>
                        </button>
                        <a href="{{ route('artwork.show', $artwork->slug) }}"
                            class="flex-1 bg-gradient-to-r from-teal-500 to-teal-600 hover:from-teal-600 hover:to-teal-700 text-white px-4 py-2.5 rounded-full text-sm font-medium transition-all duration-200 flex items-center justify-center gap-2 shadow-md hover:shadow-lg transform hover:-translate-y-0.5">
                            <i class="bi bi-eye text-sm"></i>
                            <span>{{ __('public/shop.view_details') }}</span>
                        </a>
                    </div>
                </div>
            </div>
